...
AutoSequelize.prototype.build = function (callback) {
  ...
  function processTables(tables) {
    ...
    if (self.options.tables) {
      ...
      self.targetTables = _.intersectionWith(tables, optables, compareFn);
      ...
      self.targetTables = _.differenceWith(tables, skipTables, compareFn);
    } else {
      self.targetTables = tables;
    }
...
AutoSequelize.prototype.run = function (callback) {
  ...
  var typescriptFiles = [tsHelper.def.getDefinitionFileStart(), ''];
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function (table, _callback) {
      ...
      var tableName = recase(self.options.caseModel, tableNameOrig);
      ...
      var tsTableDef = 'export class ' + className + ' extends Sequelize.Model {';
      var str;
      ...
      if (!self.options.typescript) {
        ...
        if (self.options.es6) {
          ...
          str = `'use strict'
             ...
             * @returns {Egg.Model.${className}Type}
             */
            `.replace(/            /g, '');
          str += "module.exports = app => {\n";
          str += spaces + `const sequelize = app.Sequelize;\n`
          str += spaces + `const { DataTypes } = sequelize;\n\n`
          str += spaces + `let ${className} = app.model.define('${tableName}', {\n`;
        }
      ...
      _.each(fields, function (field, i) {
        ...
        var fieldName = recase(self.options.caseProp, field);
        ...
        _.each(fieldAttr, function (attr) {
          ...
          if (attr === "foreignKey") {
            ...
            else if ((_match = _attr.match(/^(smallint|mediumint|tinyint|int|bigint|decimal)/))) {
              var length = _attr.match(/\([\d,]+\)/);
              var _type = _match[1]
              if (_type == 'int')
                _type = 'integer'
              val = 'DataTypes.' + _type.toUpperCase() + (!_.isNull(length) ? length : '');
        ...
        // if (self.options.typescript) {
        tsTableDef += tsHelper.def.getMemberDefinition(spaces, fieldName, tsVal, tsAllowNull, defaultVal);
        // }
      });
      ...
      if (hasCreatedAt || hasUpdatedAt) {
        str += spaces + spaces + "timestamps: true,\n";
        if (!hasCreatedAt)
          str += spaces + spaces + "createdAt: false,\n";
        if (!hasUpdatedAt)
          str += spaces + spaces + "updatedAt: false,\n";
      }
      ...
      // if (self.options.typescript) {
      tsTableDef += "\n" + spaces + "_attributes: " + className + "Attrs;";
      tsTableDef += "\n" + spaces + "_creationAttributes: " + className + "Attrs;";
      ...
      typescriptFiles[0] += "\n\nexport type TableNames = " + tableNames.map(t => `'${t}'`).join(' | ') + ";";
      typescriptFiles[0] += "\n\n}\n\n}"

      // typescript generate tables
      if (self.options.typescript) {
        typescriptFiles[1] = tsHelper.model.generateTableModels(_.keys(text), self.options.spaces, self.options.indentation,
          self.options.caseModel === 'c', self.options.caseFile === 'c');
      }
...
AutoSequelize.prototype.write = function (attributes, typescriptFiles, callback) {
  ...
  const tables = _.keys(attributes).filter(t => this.targetTables.some(tt => makeTableQName(tt) == t));
  const self = this;
  ...
  function createFile(table, callback) {
    ...
    var fileName = recase(self.options.caseFile, tableName);
